...
export const generateRentReceiptTemplate = (
  data: RentReceiptInterface,
): Promise<Buffer> => {
  return new Promise((resolve) => {
    ...
    doc.image(
      logoPath,
      doc.page.width - doc.page.margins.right - 75,
    ...
    doc
      .fontSize(12)
      .text(`Quittance de loyer du mois de ${data.month}`, { align: "center" });
    doc.moveDown(2);
    ...
    doc.fontSize(12).text(`Fait à ${data.signedAt}, le ${data.receiptDate}`, {
      align: "right",
    });
    doc.moveDown(1);
    ...
    doc
      .font("Helvetica-Bold")
      .text("Loyer :", { continued: true })
      .font("Helvetica")
      .text(` ${data.rentAmount} euros`);
    doc.moveDown(0.5);
    doc
      .font("Helvetica-Bold")
      .text("Provision pour charges :", { continued: true })
      .font("Helvetica")
      .text(` ${data.chargesAmount} euros`);
    doc.moveDown(0.5);
    doc
      .font("Helvetica-Bold")
      .text("Total :", { continued: true })
      .font("Helvetica")
      .text(` ${data.totalAmount} euros`);
    doc.moveDown(1);

    // Payment date
    doc
      .font("Helvetica-Bold")
      .text("Date du paiement :", { continued: true })
      .font("Helvetica")
    ...
    doc.fontSize(12).text("(Signature)", {
      align: "right",
      width: doc.page.width - doc.page.margins.right - 100,
    });
    doc.moveDown(2);
