...
describe('GET /api/documents', () => {
    ...
    it('should return 200 and list of documents', async () => {
        ...
        expect(res.body.documents[0].url).toContain('https://storage.googleapis.com/test-bucket/42/document/quittance-123.pdf');
    });
    ...
    it('should return 500 if bucket name is not defined', async () => {
        ...
        expect(res.body.message).toBe('Le nom du bucket GCP est manquant');
    });
    ...
    it('should return 500 if storage throws an error', async () => {
        ...
        expect(res.statusCode).toBe(500);
        expect(res.body.message).toBe('Erreur lors de la récupération des documents');
    });
});
