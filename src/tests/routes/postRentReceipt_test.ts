...
describe('POST /api/rent-receipt', () => {
    ...
    it('should return 400 if leaseId is missing', async () => {
        ...
        expect(res.body.message).toBe('leaseId est requis et doit être un nombre.');
    });
    ...
    it('should return 200 and PDF URL if everything works', async () => {
        ...
        (generateRentReceiptTemplate as jest.Mock).mockResolvedValue(Buffer.from('PDF'));
        ...
        expect(res.body.message).toBe('Quittance générée et enregistrée avec succès');
        expect(res.body.pdfUrl).toContain('https://storage.googleapis.com/test-bucket/42/document/');
    });

    it('should return 500 if service throws an error', async () => {
        (getRentReceiptDataFromLease as jest.Mock).mockRejectedValue(new Error('DB error'));

        const res = await request(app).post('/api/rent-receipt').send({ leaseId: 1 });
